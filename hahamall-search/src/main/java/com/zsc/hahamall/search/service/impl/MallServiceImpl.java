...
import com.zsc.hahamall.search.feign.ProductFeignService;
import com.zsc.hahamall.search.service.MallService;
import com.zsc.hahamall.search.vo.AttrResponseVo;
import com.zsc.hahamall.search.vo.BrandVo;
...
public class MallServiceImpl implements MallService {
	...
	public SearchResult search(SearchParam Param) {
		...
		try {
			// 2.执行检索请求
			SearchResponse response = restHighLevelClient.search(searchRequest, HahamallElasticSearchConfig.COMMON_OPTIONS);
	...
	private SearchRequest buildSearchRequest(SearchParam Param) {
		...
		// 1.1 must  计算分数 其查询子句必须全部被满足，逻辑相当于and
		BoolQueryBuilder boolQuery = QueryBuilders.boolQuery();
		if(!StringUtils.isEmpty(Param.getKeyword())){
			boolQuery.must(QueryBuilders.matchQuery("skuTitle",Param.getKeyword()));
		}
		// 1.2 bool - filter Catalog3Id 在filter context 下的查询子句不会计算分数且会被缓存。
	...
	private SearchResult buildSearchResult(SearchResponse response, SearchParam Param) {
		...
		if(Param.getAttrs() != null){
			List<SearchResult.NavVo> navVos = Param.getAttrs().stream().map(attr -> {
				...
				navVo.setLink("http://search.hahamall.com/list.html?" + replace);
				return navVo;
